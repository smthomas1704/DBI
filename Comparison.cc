...
/**
* It works on the current CNF object, with which GetSortOrders is called
* 
**/
...
int CNF :: GetSortOrders_new (OrderMaker &left, OrderMaker &right) {
	...
	for (int i = 0; i < numAnds; i++) {
		...
		// made it this far, so first verify that it is an equality check, greater than or less than
		if ((orList[i][0].op != Equals) && (orList[i][0].op != GreaterThan) && (orList[i][0].op != LessThan)) {
			continue;
		}
