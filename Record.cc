...
/**
 * This function examines the record passed to it and counts the number of attributes.
 * Make use of the below to figure out numAtts
 ...
 *
 **/
int Record::GetRecordNumAtts() {			
	if(((int*)bits)[0] < 3)							//If the bits do not have atleast 3 bytes, i.e 1 attribute record, then return.
		return 0;
	
	return ((((int*)bits)[1])/sizeof(int)) - 1;		//Not get the offset of the first attribute, divide it by sizeof(int) to get the
    //number of offset indicators, and subtract by 1 to remove the 1st byte which indicates the
    //total size.
}



int Record::WriteOutRecord(Schema* schema, FILE* outFile) {
    Attribute *atts = schema->GetAtts();
    
    int no_of_atts = schema->GetNumAtts();
    
    for (int i = 0; i < no_of_atts; i++)
    {
        int wrt_ptr = ((int *) bits)[i + 1];
        
        if (atts[i].myType == Int)
        {
            int *myInt = (int *) &(bits[wrt_ptr]);
            fprintf(outFile, "%d", *myInt);
        }
        
        else if (atts[i].myType == Double)
        {
            double *myDouble = (double *) &(bits[wrt_ptr]);
            fprintf(outFile, "%lf", *myDouble);
        }
        
        else if (atts[i].myType == String)
        {
            char *myString = (char *) &(bits[wrt_ptr]);
